%extend hyperdex::Client
{
    static int get_attr_name_sz(hyperdex_client_attribute *ha)
    {
    ...
    static void read_attr_name(hyperdex_client_attribute *ha,
    ...
    static void read_attr_value(hyperdex_client_attribute *ha,
    ...
    static int get_map_attr_name_sz(hyperdex_client_map_attribute *hma)
    {
    ...
    static std::string read_map_attr_name(hyperdex_client_map_attribute *hma,
                                            char *name, size_t name_sz)
    {
        std::string str = std::string(hma->attr);
        memcpy(name, str.data(), name_sz);
    ...
    static int get_attr_check_name_sz(hyperdex_client_attribute_check *hac)
    {
    ...
    static void read_attr_check_name(hyperdex_client_attribute_check *hac,
                                char *name, size_t name_sz)
    {
        std::string str = std::string(hac->attr);
        memcpy(name, str.data(), name_sz);
    ...
    static hyperdex_client_attribute *alloc_attrs(size_t attrs_sz)
    {
        return (hyperdex_client_attribute *)calloc(attrs_sz,sizeof(hyperdex_client_attribute));
    }

    static void free_attrs(hyperdex_client_attribute *attrs, size_t attrs_sz)
    {
    ...
    static hyperdex_client_map_attribute *alloc_map_attrs(size_t attrs_sz)
    {
        return (hyperdex_client_map_attribute *)calloc(attrs_sz,
                                                   sizeof(hyperdex_client_map_attribute));
    }

    static void free_map_attrs(hyperdex_client_map_attribute *attrs, size_t attrs_sz)
    {
        for (size_t i=0; i<attrs_sz; i++)
        {
            if (attrs[i].attr) free((void*)(attrs[i].attr));
    ...
    static hyperdex_client_attribute_check *alloc_attrs_check(size_t attrs_sz)
    {
        return (hyperdex_client_attribute_check *)calloc(attrs_sz,
                                                     sizeof(hyperdex_client_attribute_check));
    }

    static void free_attrs_check(hyperdex_client_attribute_check *attrs, size_t attrs_sz)
    {
        for (size_t i=0; i<attrs_sz; i++)
        {
            if (attrs[i].attr) free((void*)(attrs[i].attr));
    ...
    static int write_attr_name(hyperdex_client_attribute *ha,
    ...
    static int write_attr_value(hyperdex_client_attribute *ha,
                                    const char *value, size_t value_sz)
    {
        char *buf = NULL;
        // Note: Since hyperdex_client_attribute array was calloced
    ...
    static int write_map_attr_name(hyperdex_client_map_attribute *hma,
                                   const char *attr, size_t attr_sz)
    {
        char *buf;
    ...
    static int write_map_attr_map_key(hyperdex_client_map_attribute *hma,
                                          const char *map_key, size_t map_key_sz,
                                          hyperdatatype type)
    {
        char *buf = NULL;
        // Note: Since hyperdex_client_map_attribute array was calloced
    ...
    //
    // If hma->value is already non-NULL, then we are appending to it.
    static int write_map_attr_value(hyperdex_client_map_attribute *hma,
                                    const char *value, size_t value_sz,
                                    hyperdatatype type)
    {
        char *buf = NULL;
    ...
    static int write_attr_check_name(hyperdex_client_attribute_check *hac,
                                   const char *attr, size_t attr_sz,
                                   hyperpredicate pred)
    {
        char *buf;
    ...
    static int write_attr_check_value(hyperdex_client_attribute_check *hac,
                                        const char *value, size_t value_sz)
    {
        char *buf = NULL;
        // Note: Since hyperdex_client_attribute_check array was calloced
    ...
    //                       hac->value_sz will be 0
    //
    static int write_attr_check_value(hyperdex_client_attribute_check *hac, int64_t value)
    {
        char *buf = NULL;
    ...
    //                       hac->value_sz will be 0
    //
    static int write_attr_check_value(hyperdex_client_attribute_check *hac, double value)
    {
        char *buf = NULL;
    ...
    static hyperdex_client_attribute *get_attr(hyperdex_client_attribute *ha, size_t i)
    {
        return ha + i;
    }

    static hyperdex_client_map_attribute *get_map_attr(hyperdex_client_map_attribute *hma, size_t i)
    {
        return hma + i;
    }

    static hyperdex_client_attribute_check *get_attr_check(hyperdex_client_attribute_check *hac, size_t i)
    {
        return hac + i;
    }
};

%typemap(javacode) hyperdex::Client
%{
  java.util.HashMap<Long,Pending> ops = new java.util.HashMap<Long,Pending>();

  private String defaultStringEncoding = "UTF-8";

  public Client(String coordinator, int port, String defaultStringEncoding)
  {
  ...
  void loop() throws HyperDexClientException
  {
    SWIGTYPE_p_hyperdex_client_returncode rc_ptr = hyperdex_client.new_rc_ptr();

    long ret = loop(-1, rc_ptr);

    hyperdex_client_returncode rc = hyperdex_client.rc_ptr_value(rc_ptr);

    hyperdex_client.delete_rc_ptr(rc_ptr); // Deallocate the pointer

    if ( ret < 0 )
    {
      throw new HyperDexClientException(rc);
    }
    else
    {
  ...
  java.util.Map attrs_to_dict(hyperdex_client_attribute attrs, long attrs_sz)
                                                                throws ValueError
  {
    ByteArrayKeyedMap<Object> map = new ByteArrayKeyedMap<Object>(defaultStringEncoding);

    int sz = Client.size_t_to_int(attrs_sz);

    for ( int i=0; i<sz; i++)
    {
        hyperdex_client_attribute ha = get_attr(attrs,i);
  ...
  //
  private void write_attr_check(hyperdex_client_attribute_check hac,
  ...
  //
  // retvals at 0 - hacs (hyperdex_client_attribute_check type)
                                                                  ...
                                                                  ValueError
  {
      ...
      hyperdex_client_attribute_check hacs = null;
      long hacs_sz = 0;
      ...
      try
      {
          ...
          if ( rawChecks.size() > 0 )
          {
                                ...
                                hyperpredicate,Object>> rawCheck: rawChecks )
              {
                  ...
                  hyperdex_client_attribute_check hac = Client.get_attr_check(hacs,i++);
  ...
  byte[] getBytes(Object obj, boolean nullTerminate) throws TypeError
  {
    ...
    else
    {
        ...
        throw new TypeError("Expecting bytes in the form of byte[], ByteArray, or String (encoded with HyperDexClient's current default encoding which is set at '" + defaultStringEncoding + "'. But, instead got type: " + obj.getClass().getName());
    }
  ...
  hyperdex_client_attribute dict_to_attrs(java.util.Map attrsMap) throws TypeError,
                                                                     MemoryError
  {
    int attrs_sz = attrsMap.size();
    hyperdex_client_attribute attrs = alloc_attrs(attrs_sz);
  ...
  //
  hyperdex_client_map_attribute dict_to_map_attrs(java.util.Map attrsMap) throws TypeError,
                                                                             MemoryError
  {
    ...
    // of the hyperdex_client_map_attribute array
    //
    for (java.util.Iterator it=attrsMap.keySet().iterator(); it.hasNext();)
    {
    ...
    hyperdex_client_map_attribute attrs = alloc_map_attrs(attrs_sz);
        ...
        try
        {
            ...
            if ( operand instanceof java.util.Map )
            {
                ...
                for ( java.util.Iterator iit=map.keySet().iterator(); iit.hasNext(); )
                {
                    ...
                    hyperdex_client_map_attribute hma = get_map_attr(attrs,i_bi.longValue());
  ...
  // Synchronous methods
  //
  public java.util.Map get(Object space, Object key) throws HyperDexClientException,
                                                            TypeError,
                                                            ValueError
  {
  ...
  public boolean del(Object space, Object key) throws HyperDexClientException,
                                                      TypeError,
                                                      ValueError
  {
  ...
  //
  public Deferred async_get(Object space, Object key) throws HyperDexClientException,
                                                             TypeError,
                                                             ValueError
  {
  ...
  public Deferred async_del(Object space, Object key) throws HyperDexClientException,
                                                             TypeError
  {
