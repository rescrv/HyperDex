...
//
%ignore hyperdex_client_attribute::attr;
%ignore hyperdex_client_attribute::value;

%typemap(javacode) hyperdex_client_attribute
%{
  byte[] getAttrNameBytes()
  {
    int name_sz = Client.get_attr_name_sz(this);
    byte[] bytes = new byte[name_sz];

    Client.read_attr_name(this,bytes);
  ...
  {
    int bytes_sz = Client.size_t_to_int(getValue_sz());
  ...
  private byte[] getAttrValueBytes(long pos, int size)
  {
    ...
    Client.read_attr_value(this,bytes,pos);
    return bytes;
  }
