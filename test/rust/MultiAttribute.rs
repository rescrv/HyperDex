...
fn main() {
    ...
    let mut client = Client::new(FromStr::from_str(format!("{}:{}", args[1], args[2]).as_str()).unwrap()).unwrap();
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v1", r"ABC",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!(r"v1", r"ABC", r"v2", r"",);
                match client.get(r"kv", r"k") {
                    Ok(obj) => {
...
actual: {:?}", expected, obj);
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v2", r"123",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!(r"v1", r"ABC", r"v2", r"123",);
                match client.get(r"kv", r"k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
actual: {:?}", expected, obj);
            ...
            let expected = NewHyperObject!(r"v1", r"ABC",);
                match client.get_partial(r"kv", r"k", vec!(r"v1")) {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
actual: {:?}", expected, obj);
