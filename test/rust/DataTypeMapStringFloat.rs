...
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", NewHyperObject!("A", 3.14 as f64, "B", 0.25 as f64, "C", 1.0 as f64))) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", NewHyperObject!("A", 3.14 as f64, "B", 0.25 as f64, "C", 1.0 as f64));
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if obj != expected {
                         panic!("expected: {:?}
