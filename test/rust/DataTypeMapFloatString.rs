...
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", NewHyperObject!(0.25 as f64, "Y", 1.0 as f64, "Z", 3.14 as f64, "X"))) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", NewHyperObject!(0.25 as f64, "Y", 1.0 as f64, "Z", 3.14 as f64, "X"));
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if obj != expected {
                         panic!("expected: {:?}
