...
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", NewHyperObject!(0.25 as f64, 2 as i64, 1.0 as f64, 3 as i64, 3.14 as f64, 1 as i64))) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", NewHyperObject!(0.25 as f64, 2 as i64, 1.0 as f64, 3 as i64, 3.14 as f64, 1 as i64));
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if obj != expected {
                         panic!("expected: {:?}
