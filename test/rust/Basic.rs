...
actual: {:?}", expected, obj);
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v", r"v2",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!(r"v", r"v2",);
                match client.get(r"kv", r"k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
actual: {:?}", expected, obj);
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v", r"v3",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
