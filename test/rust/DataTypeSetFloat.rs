...
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", BTreeSet::from_iter(vec!(0.25 as f64, 1.0 as f64, 3.14 as f64).iter());)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", BTreeSet::from_iter(vec!(0.25 as f64, 1.0 as f64, 3.14 as f64).iter()););
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if obj != expected {
                         panic!("expected: {:?}
