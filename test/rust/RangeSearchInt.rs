...
fn main() {
    ...
    let mut client = Client::new(FromStr::from_str(format!("{}:{}", args[1], args[2]).as_slice()).unwrap()).unwrap();
                ...
                match client.put(r"kv", -2 as i64, NewHyperObject!(r"v", -2 as i64,)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            
                match client.put(r"kv", -1 as i64, NewHyperObject!(r"v", -1 as i64,)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            
                match client.put(r"kv", 0 as i64, NewHyperObject!(r"v", 0 as i64,)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            ...
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"k", LESS_EQUAL, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -2 as i64, r"v", -2 as i64,), NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,), NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"v", LESS_EQUAL, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -2 as i64, r"v", -2 as i64,), NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,), NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"k", GREATER_EQUAL, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,), NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,), NewHyperObject!(r"k", 2 as i64, r"v", 2 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"v", GREATER_EQUAL, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,), NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,), NewHyperObject!(r"k", 2 as i64, r"v", 2 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"k", LESS_THAN, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -2 as i64, r"v", -2 as i64,), NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"v", LESS_THAN, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -2 as i64, r"v", -2 as i64,), NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"k", GREATER_THAN, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,), NewHyperObject!(r"k", 2 as i64, r"v", 2 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"v", GREATER_THAN, 0 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,), NewHyperObject!(r"k", 2 as i64, r"v", 2 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"k", GREATER_EQUAL, -1 as i64), HyperPredicate::new(r"k", LESS_EQUAL, 1 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,), NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,), NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,)).len());
        
            let res = client.search(r"kv", vec!(HyperPredicate::new(r"v", GREATER_EQUAL, -1 as i64), HyperPredicate::new(r"v", LESS_EQUAL, 1 as i64)));
            let elems: Vec<Result<HyperObject, HyperError>> = res.iter().collect();
            assert_eq!(elems.len(), vec!(NewHyperObject!(r"k", -1 as i64, r"v", -1 as i64,), NewHyperObject!(r"k", 0 as i64, r"v", 0 as i64,), NewHyperObject!(r"k", 1 as i64, r"v", 1 as i64,)).len());
        }
