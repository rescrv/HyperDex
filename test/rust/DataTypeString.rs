...
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", "xxx",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", "xxx",);
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
actual: {:?}", expected, obj);
                ...
                match client.put("kv", "k", NewHyperObject!("v", "\xde\xad\xbe\xef",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!("v", "\xde\xad\xbe\xef",);
                match client.get("kv", "k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
