...
actual: {:?}", expected, obj);
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v", r"xxx",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!(r"v", r"xxx",);
                match client.get(r"kv", r"k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
actual: {:?}", expected, obj);
                ...
                match client.put(r"kv", r"k", NewHyperObject!(r"v", r"\xde\xad\xbe\xef",)) {
                    Ok(()) => (),
                    Err(err) => panic!(err),
                }
            let expected = NewHyperObject!(r"v", r"\xde\xad\xbe\xef",);
                match client.get(r"kv", r"k") {
                    Ok(obj) => {
                        if !sloppyCompareHyper(&obj, &expected) {
                         panic!("expected: {:?}
