...
func main() {
	...
	if !sloppyEqual(attrs, client.Attributes{"v": client.List{}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.List{}})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": client.List{float64(3.14), float64(0.25), float64(1.0)}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqual(attrs, client.Attributes{"v": client.List{float64(3.14), float64(0.25), float64(1.0)}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.List{float64(3.14), float64(0.25), float64(1.0)}})
		panic("objects not equal")
	}
