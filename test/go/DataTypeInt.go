...
func main() {
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": int64(0)}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": int64(0)})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": int64(1)})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": int64(1)}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": int64(1)})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": int64(-1)})
	if err.Status != client.SUCCESS {
	...
	err = c.Put("kv", "k", client.Attributes{"v": int64(0)})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	err = c.Put("kv", "k", client.Attributes{"v": int64(9223372036854775807)})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": int64(9223372036854775807)}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": int64(9223372036854775807)})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": int64(-9223372036854775808)})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": int64(-9223372036854775808)}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": int64(-9223372036854775808)})
		panic("objects not equal")
	}
	os.Exit(0)
}
