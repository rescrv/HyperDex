...
func main() {
	...
	err = c.Put("kv", "k", client.Attributes{"v": client.Map{int64(1): float64(3.14), int64(2): float64(0.25), int64(3): float64(1.0)}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{int64(1): float64(3.14), int64(2): float64(0.25), int64(3): float64(1.0)}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{int64(1): float64(3.14), int64(2): float64(0.25), int64(3): float64(1.0)}})
		panic("objects not equal")
	}
