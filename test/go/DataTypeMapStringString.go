...
func main() {
	...
	err = c.Put("kv", "k", client.Attributes{"v": client.Map{"A": "X", "B": "Y", "C": "Z"}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{"A": "X", "B": "Y", "C": "Z"}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{"A": "X", "B": "Y", "C": "Z"}})
		panic("objects not equal")
	}
