...
func main() {
	...
	err = c.Put("kv", "k", client.Attributes{"v": client.Set{"A", "B", "C"}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Set{"A", "B", "C"}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Set{"A", "B", "C"}})
		panic("objects not equal")
	}
