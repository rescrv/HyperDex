...
func main() {
	...
	err = c.Put("kv", "k", client.Attributes{"v": client.Map{"A": int64(1), "B": int64(2), "C": int64(3)}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{"A": int64(1), "B": int64(2), "C": int64(3)}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{"A": int64(1), "B": int64(2), "C": int64(3)}})
		panic("objects not equal")
	}
