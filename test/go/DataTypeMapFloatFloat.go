...
func main() {
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{}})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": client.Map{float64(0.25): float64(2.0), float64(1.0): float64(3.0), float64(3.14): float64(1.0)}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{float64(0.25): float64(2.0), float64(1.0): float64(3.0), float64(3.14): float64(1.0)}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{float64(0.25): float64(2.0), float64(1.0): float64(3.0), float64(3.14): float64(1.0)}})
		panic("objects not equal")
	}
