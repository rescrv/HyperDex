...
func main() {
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Set{}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Set{}})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": client.Set{float64(0.25), float64(1.0), float64(3.14)}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Set{float64(0.25), float64(1.0), float64(3.14)}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Set{float64(0.25), float64(1.0), float64(3.14)}})
		panic("objects not equal")
	}
	err = c.Put("kv", "k", client.Attributes{"v": client.Set{}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
