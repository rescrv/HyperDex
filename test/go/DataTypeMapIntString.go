...
func main() {
	...
	err = c.Put("kv", "k", client.Attributes{"v": client.Map{int64(1): "X", int64(2): "Y", int64(3): "Z"}})
	if err.Status != client.SUCCESS {
		os.Exit(1)
	}
	attrs, err = c.Get("kv", "k")
	...
	if !sloppyEqualAttributes(attrs, client.Attributes{"v": client.Map{int64(1): "X", int64(2): "Y", int64(3): "Z"}}) {
		fmt.Printf("%s %s\n", attrs, client.Map{"v": client.Map{int64(1): "X", int64(2): "Y", int64(3): "Z"}})
		panic("objects not equal")
	}
